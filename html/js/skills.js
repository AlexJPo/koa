...
$(document).ready(function() {
	$('.block').on('click', function() {
		...
		var diff = level - levelSkills[skill].current;

		$(this).addClass('active');

		if (level > levelSkills[skill].current && diff < nextStep) {
			totalPoint++;
			levelSkills[skill].current++;
			$(this).addClass('block-active');
			$(this).find('.milestone').addClass('milestone-active');
		...
		if (diff === 0) {
			...
			$(this).find('.milestone').removeClass('milestone-active');
	...
	$('.block').on('mouseover', function(){
		...
		var description = levelSkills[skillIndex].levels[levelIndex];
		...
		$('.desc-skill-text').text(description.desc);
		$('.desc-skill-textAdv').text(description.descAdv);
	...
	$('.skill-name').on('mouseover', function(){
		...
		$('.desc-skill-text').text(levelSkills[skillIndex].desc);
	}).on('mouseout' ,function(){
		$(this).removeClass('active-label');
	});
	...
	$('.skill-table-row').on('mouseover', function(){
		var skillIndex = $(this).data('skill');
		$('.desc-skill-name').text(levelSkills[skillIndex].title);
		$('.desc-skill-text').text(levelSkills[skillIndex].desc);
	...
	function checkNoPoint() {
		var l = levelSkills.length
			pointInSkill = 0;

		for (var i = 0; i < l; ++i) {
			if (levelSkills[i].current >  pointInSkill) {
				pointInSkill = levelSkills[i].current;
			}
		}

		if (pointInSkill > 0 && pointInSkill < 5) {
		...
		} else if (pointInSkill > 4 && pointInSkill < 8) {
			$('.advanced-level').addClass('skill-level-active');
			$('.master-level').removeClass('skill-level-active');
		} else if (pointInSkill > 7) {
