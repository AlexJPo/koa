...
$(document).ready(function() {
	...
	$('.ability-icon ').on('mousedown', function(e) {
		...
		var currentAbility = abilities[abilityIndex].row[rowIndex][skillIndex];
		var abilitiesPoints = abilities[abilityIndex].points;

		if (abilitiesPoints < maxPoint) {
			if (e.which === 1) {
				if (currentAbility.reqAbility) {
					var reqAbility = abilities[abilityIndex].row[currentAbility.reqAbility1][currentAbility.reqAbility2];

					if (reqAbility.currentLevel > 0 && abilities[abilityIndex].points >= currentAbility.requiresPoint) {
						if (currentAbility.currentLevel < currentAbility.maxLevel) {
							currentAbility.currentLevel++;
							abilities[abilityIndex].points++;

							updateAbilityLevel(currentAbility);
							updateCurrentLevel(currentAbility);

							$(this).find('.current-level').text(currentAbility.currentLevel);
						}
					}
				} else {
					if (abilities[abilityIndex].points >= currentAbility.requiresPoint) {
						if (currentAbility.currentLevel < currentAbility.maxLevel) {
							currentAbility.currentLevel++;
							abilities[abilityIndex].points++;
				...
				if (currentAbility.maxLevel === currentAbility.currentLevel) {
					$(this).find('.current-level').addClass('filled-level');
				}
			}

			$('.spendPoint').each(function() {
				if (abilityIndex === $(this).data('abilities')) {
					$(this).text(' (' + abilities[abilityIndex].points + ')');
				}
			});

			$('.points-spend').text(abilities[0].points + abilities[1].points + abilities[2].points)

			if (abilities[abilityIndex].points >= 5 && abilities[abilityIndex].points < 20) {
				updateTableRow(abilityIndex, 1);
			} else if (abilities[abilityIndex].points >= 20 && abilities[abilityIndex].points < 35) {
				updateTableRow(abilityIndex, 2);
			} else if (abilities[abilityIndex].points >= 35 && abilities[abilityIndex].points < 50) {
				updateTableRow(abilityIndex, 3);
			} else if (abilities[abilityIndex].points >= 50 && abilities[abilityIndex].points < 70) {
				updateTableRow(abilityIndex, 4);
			} else if (abilities[abilityIndex].points >= 70 ) {
	...
	$('.ability-icon').on('mouseover', function(){
		...
		var currentAbility = abilities[abilityIndex].row[rowIndex][skillIndex];
		var currentAbilityPoints = abilities[abilityIndex].points;
	...
	function updateCurrentLevel(obj) {
		if (obj.currentLevel > 0) {
			...
			if (obj.currentLevel === obj.maxLevel) {
				$('.ability-textAdv').html('').append('<li class="req-white">' + obj.descAdv[obj.currentLevel-1] + '</li>');
			} else {
				$('.ability-textAdv').html('').append('<li class="req-white">' + obj.descAdv[obj.currentLevel] + '</li>');
			}
		}
	}
	...
	function clearModel() {
		var l = abilities.length;
		for (var i = 0; i < l; i++) {
			var skills = abilities[i].row;
			var skillsLength = abilities[i].row.length;
